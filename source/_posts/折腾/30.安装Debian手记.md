...
sudo flatpak install -y flathub org.gnome.DejaDup                        # 备份
sudo flatpak install -y flathub org.freefilesync.FreeFileSync            # 文件同步
```
...
git config --global http.proxy http://192.168.123.1:10808
git config --global https.proxy http://192.168.123.1:10808
...
```shell
flatpak override --user com.jetbrains.GoLand --nosocket=wayland --socket=x11
flatpak override --user com.jetbrains.WebStorm --nosocket=wayland --socket=x11
```
...
# 安装路径为$HOME/software/miniconda3
# 不进行conda init
```
...
# 2. 关键：关闭自动挂起（只黑屏，不进入睡眠，服务继续运行）
gsettings set org.gnome.settings-daemon.plugins.power sleep-inactive-ac-type 'nothing'
gsettings set org.gnome.settings-daemon.plugins.power sleep-inactive-ac-timeout 0

# 3. 可选但推荐：关闭空闲时屏幕自动调暗（让锁屏界面更清晰）
