...
检查 SSH 服务是否在监听（默认端口是 22）
sudo ss -tlnp | grep ssh
修改配置以允许远程连接（如果需要）
sudo nano /etc/ssh/sshd_config
Port 22                  # 默认端口，如果你改了记得防火墙也要改
PermitRootLogin yes/no   # 是否允许 root 登录（建议用普通用户）
PasswordAuthentication yes  # 是否允许密码登录
sudo systemctl restart ssh
防火墙开放 SSH 端口（如果启用了防火墙）
...
这个命令会自动安装匹配的版本，比如当前 Debian 12（bookworm）会默认安装 NVIDIA 525 或更高版本，兼容 RTX 3060。
sudo reboot
验证驱动是否生效
nvidia-smi
...
+ [ ] 下载安装镜像：https://www.debian.org/distrib/
    + 推荐下载「完整安装映像」，在安装后期跳过在线下载，取消安装桌面相关套件，安装完成后再手动安装gnome
    + 其次是Live Gnome，但是安装的是完整版gnome，没得选择gnome的安装范围，会有烦人的小游戏
...
如果连接的是有线网络，并且安装的过程没有进行联网，安装完成后默认有线网卡是关闭的，需要手动打开
